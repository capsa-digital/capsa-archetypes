...
class Book(var volume: String? = null
    ...
    private fun addBook(context: AggregateBuilderContext) {
        val response = context.httpManager.sendHttpRequest("/requests/add-book.json",
                context.memento,
                mapOf(
                        "$.schema" to context.environment.getProperty("metrofox.schema")!!,
                        "$.host" to context.environment.getProperty("metrofox.host")!!,
                        "$.port" to context.environment.getProperty("metrofox.port")!!,
